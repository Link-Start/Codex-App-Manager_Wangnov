...
html.codex-theme-studio .cts-windows-menu-bar [data-cts-menu-region="main"] {
  ...
  const playIntro = () => {
    try {
      ...
      // optional "intro-video" motion asset (a loopback media-server URL) takes
      // priority while the static art remains its fallback when playback fails.
      ...
      // Motion path: a muted, PiP-disabled <video> overlays the static art while
      // it streams from the loopback server. Any load/play failure strips the
      // video and reveals the static intro underneath — strictly additive.
      let video = null;
      if (videoSrc) {
